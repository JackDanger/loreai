 ...
 */
import { db, withSyncApplying } from "../db";
import {
  DEFAULT_KDF_PARAMS,
  deriveKek,
  generateDek,
...
/** The synthetic scope_keys member id for an ephemeral-invite wrap (E-5-c-2 offline). Keyed by the
 * ephemeral PUBLIC key (base64), which is public and known to the accepting invitee (returned as
 * `eph_pub` by accept_scope_invite), so the invitee can locate the row to unwrap. */
export function ephemeralMemberId(ephPubB64: string): string {
  return `eph:${ephPubB64}`;
}

/**
 * OFFLINE INVITE (E-5-c-2, admin side): wrap the scope's CURRENT DEK to a caller-supplied ephemeral
 * PUBLIC key and store it as a synthetic `scope_keys` row (member `eph:<pub>`) at the current epoch.
 * The wrap row rides the normal push. The ephemeral keypair is generated by the CALLER (so it can
 * record eph_pub with the invite RPC and hold the secret for the token) and this is invoked only
 * AFTER the invite token exists — so a failed create_scope_invite never leaves an orphaned eph row.
 * The ephemeral SECRET NEVER touches this function or the server.
 *
 * Fail-closed: uses `getScopeKey(..., mint:false)` — the caller must already hold the DEK. Security:
 * the stored row is only the DEK wrapped to the eph PUBLIC key, unreadable without the secret. On
 * accept the invitee RETIRES this wrap (deletes it both sides), so the token secret has nothing left
 * to unwrap. A leaked token is only exploitable by an existing scope member (eph reads are
 * is_member-gated) — who already holds the DEK — so it grants no net capability; combined with
 * single-use + short expiry that is the accepted residual (no rotation, which is an admin-only team
 * action the editor/viewer invitee cannot perform).
 */
export async function mintEphemeralInviteWrap(
  scopeId: string,
  selfUserId: string,
  ephKeypair: { publicKey: Uint8Array },
): Promise<void> {
  const dek = await getScopeKey(scopeId, selfUserId, { mint: false });
  const epoch = currentScopeEpoch(scopeId, selfUserId);
  const wrapped = await wrapDekForMember(ephKeypair.publicKey, dek);
  const ephPubB64 = Buffer.from(ephKeypair.publicKey).toString("base64");
  putWrappedScopeKey(
    scopeId,
    ephemeralMemberId(ephPubB64),
    wrapped,
    epoch,
    Date.now(),
  );
...
/**
 * OFFLINE INVITE (E-5-c-2, invitee side): unwrap the scope DEK from the ephemeral-invite wrap using
 * the token's ephemeral SECRET, then re-wrap it to THIS device's own identity at the wrap's epoch
 * so the invitee holds a normal self-wrap. Reads the `eph:<ephPub>` row that the accept-side pull
 * delivered locally. Throws if the row is absent (pull hasn't landed) or the secret is wrong (HPKE
 * auth failure). Does NOT rotate — the caller instead RETIRES (deletes) the ephemeral wrap so the
 * token secret can never unwrap it again (rotation is an admin-only team action; deleting the spent
 * single-use wrap is the correct retirement for an editor/viewer invitee).
 */
export async function adoptEphemeralInviteWrap(
  scopeId: string,
  selfUserId: string,
  ephPubB64: string,
  ...
  const row = db()
    .query(
      "SELECT wrapped_dek AS w, key_epoch AS e FROM scope_keys WHERE scope_id = ? AND member_user_id = ? ORDER BY key_epoch DESC LIMIT 1",
    )
    .get(scopeId, ephemeralMemberId(ephPubB64)) as
    | { w: unknown; e: number }
    | undefined;
  if (!row) throw new Error("ephemeral invite wrap not found for scope");
  const dek = await unwrapDek(ephSecret, toU8(row.w)); // HPKE auth fails on a wrong/tampered secret
  const id = getAccountIdentity();
  const selfWrap = await wrapDekForMember(id.publicKey, dek);
  // Capture-suppressed: scope_keys_insert is admin-only, so an editor/viewer invitee cannot push
  // their own wrap — this row is LOCAL-ONLY (the admin's reconcile writes the remote copy). Leaving
  // it un-suppressed would enqueue an outbox row that fails RLS on push and wedges the cursor.
  withSyncApplying(() =>
    putWrappedScopeKey(scopeId, selfUserId, selfWrap, row.e, Date.now()),
  );
}

/** Delete the local `eph:<pub>` scope_keys row after it has been adopted, under capture-suppression
 * (no outbox DELETE — a non-admin invitee can't push a scope_keys delete; the remote row is retired
 * via the `retire_ephemeral_invite` RPC instead). Idempotent. */
export function deleteEphemeralInviteWrap(
  scopeId: string,
  ephPubB64: string,
): void {
  withSyncApplying(() =>
    db()
      ...
      .run(scopeId, ephemeralMemberId(ephPubB64)),
  );
}
