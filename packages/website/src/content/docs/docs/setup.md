...
- **Claude Code Desktop** — `lore setup claude-code-desktop` writes the same `~/.claude/settings.json` (the documented path the Desktop honors for Claude sessions) and, on Windows, also runs `setx ANTHROPIC_BASE_URL …` so dev/preview servers inherit it. It prints in-app Local env editor values as a fallback for Desktop builds that hit upstream bug anthropics/claude-code#67619. The in-app editor store is encrypted (Electron `safeStorage`) and cannot be written automatically.
...
lore setup claude-code-desktop # Configure Claude Code Desktop (writes settings.json + prints in-app env values)
...
| `claude-code-desktop`| `~/.claude/settings.json` (primary) + Windows `setx ANTHROPIC_BASE_URL` | Writes the documented `settings.json` `env` block the Desktop honors for Claude sessions; on Windows also `setx ANTHROPIC_BASE_URL` for dev/preview servers. Prints in-app Local env editor values as a fallback for builds hitting upstream bug anthropics/claude-code#67619 (the editor store is `safeStorage`-encrypted and cannot be written automatically). Not supported on Linux. |
...
- **Claude Code Desktop** — `lore setup claude-code-desktop` writes `~/.claude/settings.json` (the documented path the Desktop honors for Claude sessions) and, on Windows, runs `setx ANTHROPIC_BASE_URL …` for dev/preview servers. If your Desktop build does not pick up `settings.json` (upstream bug anthropics/claude-code#67619), apply the printed in-app Local env editor values as a fallback — that store is encrypted (`safeStorage`) and cannot be written automatically. Restart the Desktop after any change. Not available on Linux — use the CLI there.
