...
If you're using `lore run` with a CLI-based agent that reads env vars (Hermes Agent), you do **not** need `lore setup` — `lore run` injects the right env vars into the child process at launch. Codex, OpenCode, Claude Code, and Pi are the exceptions: each reads provider config from its own config file rather than a base-URL env var at launch. `lore setup <app>` writes the persistent config they need so you can launch them directly without going through `lore run`.
