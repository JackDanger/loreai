...
<!-- lore:019f84e7-a5a0-702e-8c7a-aba4e300f692 -->
* **DEFAULT\_REASONING\_MODEL\_BUDGET raised 8192→16384 (floor 24576)**: PR #1430 (issue #1426): raised \`DEFAULT\_REASONING\_MODEL\_BUDGET\` from 8192 to 16384, and the no-effort floor from a lower value to 24576, so the heavy claude-sonnet-5 curator pass lands on attempt 1 instead of relying on a retry. Verified by mutation testing: reverting the constants flipped 5 floor-test assertions to RED. Gotcha: config constants like this are read at gateway startup — changing the value requires a Lore Gateway restart to take effect; it is not hot-reloaded.
...
<!-- lore:019f84e3-dbfd-7a39-b7bf-6a95708de279 -->
* **anthropic-client-openai-upstream-stream test flake: startServer port race**: Under full-suite parallel load, packages/gateway/test/anthropic-client-openai-upstream-stream.test.ts:154 intermittently fails with \`TypeError: fetch failed\` / \`Caused by: Error: bad port\`, stack-traced to packages/core/test/setup.ts:80. Trap: setup.ts:80 (\`return realFetch(input, init)\`) looks like the culprit since it's the failing frame, but it only forwards to real fetch when the URL isn't MODELS\_DEV\_API — it doesn't rewrite/break the URL. Actual root cause: startServer() returns before server.port is actually bound/populated, so \`baseURL = http://127.0.0.1:${server.port}\` gets a falsy/invalid port; likely worsened by DB churn (resetPipelineState) under parallel suite load. Passes reliably in isolation, only reproduces under full-suite parallel runs. Filed as issue #1429 with suggested fix: await port binding before startServer returns.
...
<!-- lore:019f84e7-a5cf-73c1-81b4-25eb9531e3af -->
* **Test flake #1429: startServer port race is not a real regression signal**: Trap: a failing test tied to \`startServer\` port binding during a full suite run looks like a regression caused by the current change, since it appears right after the change lands. Fix: this is a known flaky test (filed as issue #1429, a port race in server startup) — re-run the full suite in isolation; if it passes clean (confirmed: 6428 passed / 226 skipped, exit 0) the original failure was the flake, not a real regression. Always re-run before attributing a failure to your change when the failing test is startup/port-related.
...
<!-- lore:019f84e3-dc2b-730e-8f3d-6c199dce73c4 -->
* **setup.ts silenceStderr reset in afterEach — never crosses test boundary**: packages/core/test/setup.ts resets silenced-stderr state via \`silenceStderr(false)\` in an \`afterEach\` hook (documented in the file's own comments, lines 83-89) so silenced state never leaks across a test boundary. Any new stderr/stdout-silencing helper added to this shared test setup must follow the same per-test reset convention.
...
<!-- lore:019f84e3-dc58-72ff-bab2-4bd75d10000b -->
* **File issue for reproducible flake only after searching open+closed by filename**: When asked to file a GitHub issue for a test flake, search both open AND closed issues broadly by the failing test's filename (not just error-message keywords) before concluding none exists — narrow keyword search alone risks missing/duplicating. Once confirmed novel, the issue body should include the exact error text, file:line of the failure, a root-cause hypothesis, and a suggested fix.

<!-- lore:019f84e7-5474-7b92-a0a2-d994df467f8d -->
* **Follow strict verify-then-ship workflow for bug-fix PRs**: For each bug fix (e.g., reasoning-budget/floor changes in llm-adapter.ts), always follow this pipeline before considering it done: (1) root-cause with production log evidence, (2) implement minimal fix, (3) update/add tests and mutation-verify them by reverting the fix and confirming tests go RED for every distinct invariant introduced (gating conditions, clamp bounds, floor values), then restore the fix and confirm the tree is clean/tests pass; never skip straight from implementation to commit without this proof, (4) run typecheck + lint + full test suite, (5) if an unrelated test fails, isolate and re-run it alone to confirm it's a pre-existing flake (file a separate issue) rather than blaming the fix, (6) commit, push a descriptively-named branch, open a PR referencing the originating issue/prior PRs plus evidence, (7) launch adversarial code review as a background/async agent (never block or poll synchronously), (8) poll CI/Seer and merge only when green and review is clear, then arm auto-merge. Use \<pty\_exited> notifications instead of sleep+pty\_read loops; note fixes require a service restart to take effect.
...
* **Invariant-check action is advisory-only and never fails the build**: The invariant-check action is advisory-only: it never fails the build and never blocks a PR — it reports findings as annotations and a job summary; humans decide. Capture the exit code and always continue; a non-zero exit from the CLI means a TOOLING failure (no range) — log it and pass. This is a directive preference.
...
* **Invariant-check judge: never produce false violation, never throw uncaught, never fail build**: Invariant-check judge: never produce a false \`violates:true\` finding, never throw uncaught (including never throwing on the request path), and never fail the build. This is a directive preference.
...
* **PR reviews: synchronous, isolated, adversarial per REVIEW.md**: When asked to review a PR/bug fix, do it now, in the same turn — never defer to a later session. Follow quality/REVIEW.md adversarial discipline: work in an isolated jj/git worktree (never mutate the main tracked tree; if told 'just report, don't fix,' only mutate scratch/temp copies). Identify concrete numeric/logic invariants to verify (e.g., clamp bounds never exceed output limits) and cross-check every changed test assertion against underlying data. Apply mutation testing: revert the fix, confirm the regression test fails ('never trust the fix without seeing it fail'), then restore and confirm it passes — verify the tree is restored exactly (sha256 hash check) to the original PR diff. Run the full gate (test, typecheck, lint, format:check) and report exit codes. Cite findings as file:line with severity (BLOCKER/SHOULD-FIX/NIT), answer any explicit enumerated questions, and conclude with an explicit MERGE/DO-NOT-MERGE decision plus residual risks — never a bare 'LGTM'.
