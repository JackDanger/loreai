...
* **Cache bust detection via prefix ID hash but no rate tracking**: Gradient tracks cache busts via prefix hash (first 5 message IDs + layer). Logs rate: bustCount/transformCount. Alerts stderr when rate >50% after 20+ calls. Busts cost ~$3.75/MTok × context\_size.
...
* **Plugin auto-detects gateway and configures provider baseURLs via config hook**: Plugin auto-detects gateway (health check \`http://localhost:6969/health\`, 1s timeout). If responding, routes all provider requests through gateway via \`baseURL: http://localhost:6969\`. Sets \`LORE\_GATEWAY\_MODE=1\` when active.
