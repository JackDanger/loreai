-- Migration 0014: extend server-side eviction to entity_aliases + entity_relations
-- (#1191b PR2a). Same relief-valve as 0013 (knowledge), by RECENCY.
--
-- entity_aliases (cap 300) and entity_relations (cap 300) still hard-pause at the free-tier
-- row cap. Both carry `updated_at`, so on an INSERT that would exceed the row cap we evict
-- the OLDEST live row for the scope instead of rejecting. No incoming-value guard is needed:
-- the trigger is BEFORE INSERT, so the incoming row is not yet in the table and can NEVER be
-- its own victim — it always survives (a timing invariant, independent of its `updated_at`;
-- note a client can push an INSERT with an arbitrary updated_at). Contrast knowledge, where
-- the value is confidence. These are LEAF tables (nothing references an alias/relation row),
-- so eviction is a plain hard DELETE — no cascade.
--
-- Everything else is inherited from 0013 verbatim: FREE-tier only, ROW cap only (byte-cap
-- still raises), hard delete (cache eviction, not a peer-propagating tombstone; local is
-- source of truth), and the per-scope hourly circuit breaker (`sync_eviction_budget` +
-- `sync_config`). `entities` is deferred to PR2b — its value (ref-count) is not
-- self-contained and a new entity starts low-value, so it needs the synced `sync_rank`
-- column + an incoming-value guard.
--
-- The only change from 0013 is the eviction block: it now fires for
-- {knowledge, entity_aliases, entity_relations} and branches per-table (knowledge keeps
-- its base_confidence + cascade path; the entity tables use recency + a plain delete).
...
begin
  ...
  -- freed slot. Per-table value: knowledge → knowledge_meta.base_confidence (ties: oldest
  -- touched); entity_aliases/entity_relations → recency (oldest updated_at). Bounded by
  -- the per-scope hourly circuit breaker.
  if tg_op = 'INSERT' and user_tier = 'free'
     and tg_table_name in ('knowledge', 'entity_aliases', 'entity_relations')
    ...
    if evb_count < evb_cap then
      if tg_table_name = 'knowledge' then
        ...
        -- value (protected), NOT low. (1) matches knowledge_current's COALESCE(confidence,
        -- 1.0) — meta-less = default 1.0 everywhere; (2) meta-less is the NORMAL transient
        -- state during a push (client pushes knowledge before its knowledge_meta), so 0.0
        -- would evict a JUST-pushed fresh entry before its meta arrives. Orphans are the
        -- reaper's (#909) job.
